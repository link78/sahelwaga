...
export default function NewSalesOrderPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`${API_URL}/clients?pageSize=200`, { headers }).then(r => r.json()),
  ...
  async function submit(e: React.FormEvent) {
    ...
    const token = window.localStorage.getItem('sahelwaga.access');
    try {
      const r = await fetch(`${API_URL}/sales-orders`, {
        method: 'POST',
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <h1 className="font-serif text-3xl font-semibold">New Sales Order</h1>
      {error && <p className="mt-4 text-sm text-red-600">{error}</p>}

      <form onSubmit={submit} className="mt-6 space-y-6">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <label className="block text-sm font-medium text-brand-neutral-700">Client *</label>
          ...
          <div>
            ...
            <select value={currency} onChange={e => setCurrency(e.target.value)}
              className="mt-1 w-full rounded-md border border-brand-neutral-200 px-3 py-2 text-sm">
              {['XOF', 'USD', 'EUR'].map(c => <option key={c} value={c}>{c}</option>)}
            </select>
          </div>
        </div>
